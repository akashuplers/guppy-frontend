          ...
          return (
              <div className="text-light d-inline-flex flex-wrap gap gx-2">
                {console.log("record?.status",record?.status)}
                  ...
                  <span className='me-2'>{record?.status ? record?.status:"NA"}</span>
            </div>
          )
      }
  },
            ...
            return (
              <div style={{ display: 'flex', gap: '15px' }}>
                  ...
                  className={`flex items-center justify-center p-2 rounded 
                    ${(record?.status !== "completed" || record?.status === "failed") ? "cursor-not-allowed opacity-50" : "cursor-pointer"}`} 
                >
                ...
                >
                  ...
                  <>
                   <button
                  title="Delete story"
                  disabled={record?.status == "processing" || record?.status == undefined}
                ...
                className={`flex items-center justify-center p-2 rounded 
                  ${(record?.status == "processing" || record?.status == undefined) ? "cursor-not-allowed opacity-50" : "cursor-pointer"}`} 
                >
                <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" viewBox="0 0 24 24" fill="none">
