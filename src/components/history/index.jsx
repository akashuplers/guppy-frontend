...
const UserHistory = () => {
  ...
  useEffect(() => {
    if (shareIds) handleSharedUsers();
  }, [shareIds, updatedShareIds]);

  const onClick = ({ key }) => {
    if (key === "all") setFilteredStories(stories);
    else
  ...
  const getItems = () => {
    const uniqueStories = Array.from(
      new Set(stories.map((story) => story.story_world))
    ).map((storyWorld) => ({
      label: storyWorld,
      key: storyWorld,
    }));
        ...
        <Dropdown menu={{ items, onClick }}>
          <a onClick={(e) => e.preventDefault()}>
            <Space>
              Story World
              <FilterOutlined
                style={{ fontSize: "12px", color: "#555", paddingTop: "5px" }}
              />
            </Space>
          </a>
        </Dropdown>
      ...
      render: (val, record) => {
        return (
          <div className="text-light d-inline-flex flex-wrap gap gx-2">
            ...
            <span className="me-2">
              {record?.status ? record?.status : "NA"}
            </span>
          </div>
        );
      },
          ...
          <div style={{ display: "flex", gap: "15px" }}>
              ...
              <button
                ...
                onClick={() => {
                  localStorage.setItem(
                    "storyId",
                    JSON.stringify(record?.story_id)
                  );
                  navigate("/home");
                }}
            ...
            <>
              <button
                title="Delete story"
                disabled={
                  record?.status == "processing" || record?.status == undefined
                }
                onClick={() => {
                  setSelectedStoryId(record?.story_id);
    ...
    <>
      <SidebarWithHeader>
        <div className="w-full">
          {/* head */}
          <div className="flex flex-col md:flex-row items-center justify-between gap-2 md:gap-4 w-full p-4">
            <p className="text-lg sm:text-xl md:text-3xl mt-1 mb-2 md:mb-0 font-medium text-center md:text-left">
              User History
            </p>
            <button
              title="Click to Refresh the table"
              onClick={fetchStories}
              disabled={isLoading}
              className="flex items-center justify-center bg-gray-100 hover:bg-gray-200 text-black rounded-md p-2 disabled:opacity-50"
            >
